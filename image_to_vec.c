...
static CLICMDARGDEF farg[] = {
    ...
    {CLIARG_IMG, ".inpixi", "input pixel index image", "pixi", CLIARG_VISIBLE_DEFAULT, (void **)&inpixiname, NULL},
    {CLIARG_IMG, ".inpixmult", "input pixel mult image", "pixmult", CLIARG_VISIBLE_DEFAULT, (void **)&inpixmultname,
     NULL},
    {CLIARG_STR, ".outvec", "output vector image", "vecim", CLIARG_VISIBLE_DEFAULT, (void **)&outvecname, NULL}};
...
//
//
errno_t linopt_imtools_image_to_vec(const char *__restrict ID_name, const char *__restrict IDpixindex_name,
                                    const char *__restrict IDpixmult_name, const char *__restrict IDvec_name,
                                    imageID *outID)
{
    DEBUG_TRACE_FSTART();
    ...
    if (naxisin < 3)
    {
        ...
        for (long k = 0; k < NBpix; k++)
        {
            data.image[IDvec].array.F[k] =
                data.image[IDpixmult].array.F[k] * data.image[ID].array.F[data.image[IDpixindex].array.SI64[k]];
        }
    }
    else
    {
        ...
        {
            FUNC_CHECK_RETURN(create_2Dimage_ID(IDvec_name, NBpix, data.image[ID].md[0].size[2], &IDvec));
        ...
        {
            FUNC_CHECK_RETURN(create_2Dimage_ID(IDvec_name, NBpix * 2, data.image[ID].md[0].size[2], &IDvec));
            ...
            for (uint32_t kk = 0; kk < data.image[ID].md[0].size[2]; kk++)
                for (long k = 0; k < NBpix; k++)
                {
                    data.image[IDvec].array.F[kk * NBpix * 2 + 2 * k] =
                        data.image[IDpixmult].array.F[k] *
                        data.image[ID].array.CF[kk * sizexy + data.image[IDpixindex].array.SI64[k]].re;
                    data.image[IDvec].array.F[kk * NBpix * 2 + 2 * k + 1] =
                        data.image[IDpixmult].array.F[k] *
                        data.image[ID].array.CF[kk * sizexy + data.image[IDpixindex].array.SI64[k]].im;
                }
        }
    }
