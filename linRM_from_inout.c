...
static CLICMDARGDEF farg[] = {
    {CLIARG_IMG, ".inimname", "input image", "im", CLIARG_VISIBLE_DEFAULT, (void **)&inputimname, NULL},
    {CLIARG_IMG, ".inmaskname", "mask image", "mask", CLIARG_VISIBLE_DEFAULT, (void **)&inmaskname, NULL},
    {CLIARG_IMG, ".mrespimname", "measured response images", "mresp", CLIARG_VISIBLE_DEFAULT, (void **)&mrespimname,
     NULL},
    {CLIARG_STR, ".outRM", "output RM image", "ourRM", CLIARG_VISIBLE_DEFAULT, (void **)&outRMimname, NULL}};

static CLICMDDATA CLIcmddata = {"lincRMiter", "estimate response matrix from input and output", CLICMD_FIELDS_DEFAULTS};
...
#ifdef HAVE_MAGMA
    CUDACOMP_magma_compute_SVDpseudoInverse("pokeM", "pokeMinv", SVDeps, insize, "VTmat", 0, 0, 1.e-4, 1.e-7, 0, 64,
                                            NULL);
#else
                ...
                for (ii_out = 0; ii_out < xsizeout; ii_out++)
                    for (jj_out = 0; jj_out < ysizeout; jj_out++)
                    {
                        data.image[IDtmp].array.F[jj_out * xsizeout + ii_out] +=
                            ...
                            data.image[IDRM]
                                .array.F[(jj_in * xsizein + ii_in) * xsizeout * ysizeout + jj_out * xsizeout + ii_out];
                    }
            }
        for (ii_out = 0; ii_out < xsizeout; ii_out++)
            for (jj_out = 0; jj_out < ysizeout; jj_out++)
            {
                tmpv1 = data.image[IDtmp].array.F[jj_out * xsizeout + ii_out] -
                        data.image[IDout].array.F[kk * xsizeout * ysizeout + jj_out * xsizeout + ii_out];
                fitval += tmpv1 * tmpv1;
